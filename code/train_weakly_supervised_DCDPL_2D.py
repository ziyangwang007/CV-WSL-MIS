...
parser.add_argument('--exp', type=str,
                    default='ACDC/Densely_Dense_Pseudo_Label', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    modelA = ViT_seg(config, img_size=args.patch_size,
                     num_classes=args.num_classes).cuda()
    modelA.load_from(config)


    ema_modelA = ViT_seg(config, img_size=args.patch_size,
                     num_classes=args.num_classes).cuda()
    ema_modelA.load_from(config)

    for param in ema_modelA.parameters():
        param.detach_()

    modelB = create_model()
    ema_modelB = create_model(ema=True)
...
# # # # # # # # # # # # # # # # # # # # # # 
            ...
            # supervised_loss = loss_ceA + loss_ceB
# # # # # # # # # # # # # # # # # # # # # # 
            ...
            with torch.no_grad():
                ...
                ema_outputA = ema_modelA(ema_inputs)
                ema_outputB = ema_modelB(ema_inputs)
                ema_output_softA = torch.softmax(ema_outputA, dim=1)
                ema_output_softB = torch.softmax(ema_outputB, dim=1)

            consistency_weight = get_current_consistency_weight(iter_num // 1000)

            consistency_lossA = torch.mean(
                (outputs_softA - ema_output_softA) ** 2)
            consistency_lossB = torch.mean(
                (outputs_softB - ema_output_softB) ** 2)

            consistency_lossAB = torch.mean(
                (outputs_softB - ema_output_softA) ** 2)
            consistency_lossBA = torch.mean(
                (outputs_softA - ema_output_softB) ** 2)

            # if iter_num < 1000:
            #     consistency_loss = 0.0
            # else:
            #     consistency_loss = consistency_lossA + consistency_lossB + consistency_lossAB + consistency_lossBA
# # # # # # # # # # # # # # # # # # # # # # 

            beta = random.random() + 1e-10

            pseudo_supervision = torch.argmax(
                (beta * outputs_softA + (1.0-beta) * outputs_softB), dim=1)
            ...
            model1_loss = loss_ceA + consistency_lossA + consistency_lossBA + loss_pse_supA
            model2_loss = loss_ceB + consistency_lossB + consistency_lossAB + loss_pse_supB
            ...
            update_ema_variables(modelA, ema_modelA, 0.99, iter_num)
            update_ema_variables(modelB, ema_modelB, 0.99, iter_num)
            ...
            logging.info(
                'iteration %d : loss : %f, loss_ceA: %f, loss_ceB: %f' %
                (iter_num, loss.item(), loss_ceA.item(), loss_ceB.item()))
